...
var Level = Base.extend({
	...
	reload: function() {
		var settings = {};
		this.pause();
		var fail = document.getElementById('idiot-fail');
		...
		if (typeof window.__deathStreak === 'undefined') window.__deathStreak = 0;
		if (settings.lifes < 0) {
			window.__deathStreak = 0;
		} else {
			window.__deathStreak++;
			if (window.__deathStreak >= 2) {
				if (!window.__idiotModeOn) {
					var prompt = document.getElementById('idiot-prompt');
					if (prompt) prompt.classList.remove('hidden');
				}
				if (window.__soundMgr && window.__soundMgr.unlockIdiotMode)
					window.__soundMgr.unlockIdiotMode();
			}
		}
			...
			for(var j = je; j >= js; j--) {
				...
				if(obj) {
					if(obj instanceof Item && isHero && (blocking === ground_blocking.bottom || obj.blocking === ground_blocking.none)) {
						// previously played 'bump' sound here; now just activate without sound
						obj.activate(this);
					}
...
var Mario = Hero.extend({
	...
	jump: function() {
		...
		var mult = (window.__idiotModeOn ? 3 : 1);
		this.vy = constants.jumping_v * mult;
	},
	move: function() {
		this.input(keys);		
		this._super();
